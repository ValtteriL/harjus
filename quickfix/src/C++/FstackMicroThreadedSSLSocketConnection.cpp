...
namespace FIX {
FstackMicroThreadedSSLSocketConnection::FstackMicroThreadedSSLSocketConnection(socket_handle socket, SSL *ssl, Sessions sessions, Log *pLog)
    : m_socket(socket),
      m_ssl(ssl),
      m_pLog(pLog),
      m_sessions(sessions),
...
bool FstackMicroThreadedSSLSocketConnection::connect() { return socket_connect(getSocket(), m_address.c_str(), m_port) >= 0; }
...
bool FstackMicroThreadedSSLSocketConnection::readMessage(std::string &message) EXCEPT(SocketRecvFailed) {
  try {
...
bool FstackMicroThreadedSSLSocketConnection::setSession(const std::string &message) {
