    ...
    private:
        void connect()
        {
                    ...
                    0))
            {
                ...
                {
                    throw ConfigError(
                        std::string("Unable to connect to mysql database: ") + "'" + m_connectionID.getDatabase() + "': " + m_connectionID.getUser() + '@' + m_connectionID.getHost() + ":" + std::to_string(port) + " [" + mysql_error(m_pConnection) + "]");
                }
            }
