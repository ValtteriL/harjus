...
namespace
{
    auto const seqNumFileFormat = "%" + std::to_string(std::numeric_limits<uint64_t>::digits10 + 1) + "." + std::to_string(std::numeric_limits<uint64_t>::digits10 + 1) + SCNu64;
...
namespace FIX
{
    ...
    void FileStore::populateCache()
    {
        ...
        if (headerFile)
        {
            ...
            {
                std::pair<NumToOffset::iterator, bool> it = m_offsets.insert(NumToOffset::value_type(msgSeqNum, std::make_pair(offset, size)));
