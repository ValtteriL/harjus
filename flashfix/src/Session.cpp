...
#define LOGEX(method)              \
    ...
    void Session::nextLogon(const Message &logon, const UtcTimeStamp &now)
    {
        ...
        if (isTargetTooHigh(msgSeqNum) && !resetSeqNumFlag)
        {
            ...
            {
                m_state.onEvent(
                    "Expecting retransmits FROM: " + SEQNUM_CONVERTOR::convert(getExpectedTargetNum()) + " TO: " + SEQNUM_CONVERTOR::convert(msgSeqNum - 1));
        ...
        if (sendRetransmitsAfterLogon)
        {
            ...
            m_state.onEvent(
                "Sending retransmits due to received NextExpectedMsgSeqNum is too low. FROM: " + SEQNUM_CONVERTOR::convert(beginSeqNo) + " TO: " + SEQNUM_CONVERTOR::convert(endSeqNo));
    ...
    void Session::nextSequenceReset(const Message &sequenceReset, const UtcTimeStamp &now)
    {
        ...
        {
            m_state.onEvent(
                "Received SequenceReset FROM: " + SEQNUM_CONVERTOR::convert(getExpectedTargetNum()) + " TO: " + SEQNUM_CONVERTOR::convert(newSeqNo));
    ...
    void Session::nextResendRequest(const Message &resendRequest, const UtcTimeStamp &now)
    {
        ...
        m_state.onEvent(
            "Received ResendRequest FROM: " + SEQNUM_CONVERTOR::convert(beginSeqNo) + " TO: " + SEQNUM_CONVERTOR::convert(endSeqNo));

        std::string beginString = m_sessionID.getBeginString();
        if ((beginString >= FIX::BeginString_FIX42 && endSeqNo == 0) || (beginString <= FIX::BeginString_FIX42 && endSeqNo == 999999) || (endSeqNo >= getExpectedSenderNum()))
        {
    ...
    auto Session::sendRaw(Message &message, SEQNUM num) -> bool
    {
        ...
        try
        {
            ...
            if (Message::isAdminMsgType(msgType))
            {
                ...
                if (msgType == MsgType_Logon || msgType == MsgType_Logout || msgType == MsgType_ResendRequest || msgType == MsgType_SequenceReset || isLoggedOn())
                {
                    send(messageString);
                }
            }
    ...
    void Session::generateResendRequest(const BeginString &beginString, const MsgSeqNum &msgSeqNum)
    {
        ...
        m_state.onEvent(
            "Sent ResendRequest FROM: " + SEQNUM_CONVERTOR::convert(beginSeqNo) + " TO: " + SEQNUM_CONVERTOR::convert(endSeqNo));
    ...
    void Session::generateReject(const Message &message, int err, int field)
    {
        ...
        if (beginString >= FIX::BeginString_FIX42)
        {
            ...
            if ((beginString == FIX::BeginString_FIX42 && err <= SessionRejectReason_INVALID_MSGTYPE) || beginString > FIX::BeginString_FIX42)
            {
    ...
    auto Session::verify(const Message &msg, bool checkTooHigh, bool checkTooLow) -> bool
    {
        ...
        try
        {
            ...
            if ((checkTooHigh || checkTooLow) && m_state.resendRequested())
            {
                ...
                {
                    m_state.onEvent(
                        "ResendRequest for messages FROM: " + SEQNUM_CONVERTOR::convert(range.first) + " TO: " + SEQNUM_CONVERTOR::convert(range.second) + " has been satisfied.");
                    m_state.resendRange(0, 0);
                }
            }
        }
    ...
    void Session::doTargetTooHigh(const Message &msg)
    {
        ...
        m_state.onEvent(
            "MsgSeqNum too high, expecting " + SEQNUM_CONVERTOR::convert(getExpectedTargetNum()) + " but received " + SEQNUM_CONVERTOR::convert(msgSeqNum));
        ...
        if (m_state.resendRequested())
        {
            ...
            {
                m_state.onEvent(
                    "Already sent ResendRequest FROM: " + SEQNUM_CONVERTOR::convert(range.first) + " TO: " + SEQNUM_CONVERTOR::convert(range.second) + ".  Not sending another.");
                return;
            }
        }
    ...
    void Session::next(const Message &message, const UtcTimeStamp &now, bool queued)
    {
        ...
        try
        {
            ...
            const DataDictionary &sessionDataDictionary = m_dataDictionaryProvider.getSessionDataDictionary(m_sessionID.getBeginString());

            if (m_sessionID.isFIXT() && message.isApp())
            {
                ApplVerID applVerID = m_targetDefaultApplVerID;
                header.getFieldIfSet(applVerID);
                const DataDictionary &applicationDataDictionary = m_dataDictionaryProvider.getApplicationDataDictionary(applVerID);
