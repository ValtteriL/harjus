...
{
    // By default, the balance doesn't contain key
    Balance balance;
    std::string currency = "BTC";

    EXPECT_EQ(balance.getBalances().count(currency), 0);

    // Add 1 BTC to the balance
    balance.updateBalance(currency, PreciseNumber{"1"});
    EXPECT_EQ(balance.getBalances().count(currency), 1);
    ...
    EXPECT_EQ(balance.getBalances().count(currency2), 0);
}
