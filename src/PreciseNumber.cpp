...
{
    // Use cpp_dec_float_50 for intermediate calculation to avoid int overflow
    bm::cpp_dec_float_50 decimal{amount};
    decimal *= kPrecision;
    // Convert to string first to avoid boost's int-based conversion path
    smallestUnit = bm::mpz_int{decimal.str()};
}
